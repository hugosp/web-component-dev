...
const _sfc_main = {
  ...
  mounted() {
    this.code = this.$el.parentNode.querySelector("slot").assignedNodes().map((i) => i.outerHTML || i.textContent).filter((i) => i != "").join("\n");
    this.$nextTick(() => Prism$1.highlightElement(this.$refs.code));
  },
  computed: {
    codeClass() {
      return {
  ...
  return openBlock(), createElementBlock(Fragment, null, [
    ...
    createBaseVNode("pre", null, [
      createBaseVNode("code", {
        ref: "code",
        class: normalizeClass($options.codeClass)
      }, toDisplayString($data.code), 3)
    ])
  ], 64);
}
